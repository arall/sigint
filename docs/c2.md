...
- **High-volume scanners overwhelm the mesh.** The agent enqueues one DET per detection. BT and WiFi produce dozens of detections per second in a populated area; LoRa drains at ~6 s per send. Don't run `bt` or `wifi` over the mesh in such environments — the outbox grows unboundedly. It's a physics limit of the radio, not a software gap. See [troubleshooting.md](troubleshooting.md#bt--wifi-scanners-over-mesh-dont).
