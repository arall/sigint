...
Every dashboard endpoint — Live, Log, Timeline, Devices, and all category tabs — answers from direct SQL queries against the `.db` files on disk. There is no in-memory deque or per-type counter anywhere in the read path; the tailer keeps a 2-second cache of the Live-tab state (`fetch_live_state`) just so SSE broadcasts don't block on the ~200 ms aggregation query. Queries union across every `.db` file in the output directory, which is important because standalone scanner subprocesses (`sdr.py pmr`, `adsb`, `ais`, etc.) write to their own detection files separate from the main server file. Category tabs accept an optional time window (`?window=<hours>`, default 6 h, capped at 7 days) and auto-refresh every 3 s while visible. A **Session** dropdown in the header lets you scope the category tabs to a single historical `.db` for post-hoc browsing; Live / Log / Timeline / Devices always reflect the active session.
