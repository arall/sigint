...
- Server-side category pager: `/api/cat/<name>` accepts `offset`, `limit`, `type`, `channel`, `audio`, `transcript`. The server runs the loader, applies filters, and slices — only one page (50 rows) of JSON crosses the wire per refresh. Response also ships `filters_available.{types,channels}` so dropdowns stay stable when the user narrows a filter. Auto-refresh is cheap on long sessions now.
- Jammer / broadband-interference scanner (`sdr.py jammer`): hops across a configurable band list (GPS L1, Meshtastic EU, ISM 915, GSM-900 DL, Marine VHF by default), samples ~1 s of IQ per band, and fires a `signal_type="jamming"` detection when the median noise floor stays elevated with broadband character (spectral flatness > 0.5) for ≥ 3 consecutive revisits. Per-band baseline acquired at startup and persisted to `output/jammer_baseline.json` so a jammer active at boot time doesn't calibrate itself out. Deliberately rejects narrowband peaks (loud emitters aren't jammers) and spoofing (needs protocol-level integrity checks — out of scope). Dashboard exposes it as a new **Jamming** sub-tab under **Signals** with elevation-coded row colouring.
- Opportunistic jammer inference (`sdr.py jammer-detect`): complements the live scanner by walking stored `noise_floor_db` series — PMR, BLE, WiFi, ADS-B, everything that already logs a noise floor — and flagging time segments where a `(node, signal_type)` stream stays elevated against its rolling 1-hour baseline. No SDR required. Reuses `dsp.jammer.decide` so fire/clear semantics (consec hits, hysteresis exit) match the live scanner; no flatness available post-hoc, so the analyzer inherits the scanner's median-based noise-floor estimate as its "broadband-ish" proxy. Emits `signal_type="jamming-inferred"` rows that show up on the same Jamming tab with an amber source badge.
- WiFi rogue-AP detection — two new parsers on the existing monitor-mode adapter. `parsers/wifi/deauth.py` bucketed-window spike detector on `Dot11Deauth` / `Dot11Disas` (default ≥ 5 frames / 10s from the same source, 30s cooldown to suppress flood-per-packet noise) — flags `aireplay-ng`/`mdk4`-style client-kick attacks as `WiFi-Deauth`. `parsers/wifi/evil_twin.py` tracks `ssid → {first-5-octet BSSID prefix}` per session and emits `WiFi-EvilTwin` when a known SSID reappears on a different prefix; metadata flags `crypto_mismatch` (legit WPA2 → rogue open) and `manufacturer_mismatch` so the high-signal cases stand out from chain-hotel false positives. New **Rogue** sub-tab under **Signals** merges both into one table. Opt-in for the central server via `deauth` / `evil_twin` parser names in `server.json`; `sdr.py wifi` (standalone) wires them by default.
...
- **GPS lock-loss detection** — upgrade from the current spectrum-only jammer coverage to actual satellite-lock tracking on L1, so 5 dB jammers (sub-threshold for spectrum-based) are caught. Requires a real GPS decoder chain.
...
- **Passive radar (non-cooperative targets)** — detect things that *don't* transmit (drones without RemoteID, vehicles, people) by using a third-party illuminator of opportunity (FM broadcast, DVB-T, LTE, WiFi) as the "transmitter" and measuring reflections. Reference channel on the direct path, surveillance channel(s) on the reflection path, cross-ambiguity function → range-Doppler map, ECA-B / Wiener clutter cancellation, 2-D CFAR, bistatic-geometry inversion to lat/lon. Would fill the drone-detection gap left by RemoteID-only coverage (DJI only beacons when armed; most home-brew / hostile drones beacon nothing). **Not a fit for current architecture**: needs a coherent multi-channel receiver (KrakenSDR, 5× RTL2832 sharing one LO, or multiple Plutos + GPS-DO — HackRF and stock RTL-SDR V4 are both single-channel / non-coherent), a continuous raw-IQ sync path instead of the current frame-level parser pipeline, and a heavy DSP stage (CAF is O(N²) per dwell) that a Pi 5 likely can't do real-time for DVB-T bandwidths. New scanner (`scanners/passive_radar.py`) plus a new `SignalDetection` variant carrying bistatic range / Doppler / track ID. Cheapest entry point is KrakenSDR + FM broadcast illuminator (ubiquitous, but 100 kHz bandwidth → coarse range resolution). Reference impl to study: [ChiefGokhlayeh/pluto-sdr-pr](https://github.com/ChiefGokhlayeh/pluto-sdr-pr) (2× coherent Pluto, LTE illuminator, archived 2024).
...
- **GSM downlink L3 decode (TMSI/paging harvest)** — upgrade from the current `sdr.py gsm` uplink *energy* detection (`parsers/cellular/gsm.py` counts TDMA bursts, decodes nothing) to actually demodulating the downlink C0/BCCH carrier and reading clear-text signaling. Same subprocess-decoder pattern as dump1090 / rtl_ais / dsd-fme: spawn `grgsm_livemon_headless` (gr-gsm) on a downlink ARFCN → GSMTAP on UDP 4729 → a `GSMTAPParser` extracts identities and cell info into `SignalDetection` metadata. Yield is **TMSIs in volume** plus BCCH System Information (cell id, LAC, MCC/MNC, neighbor list) and paging activity — useful for presence/activity mapping, and feeds the existing **IMSI catcher detection** idea (observed cell IDs vs OpenCelliD). Caveats: IMSIs are sparse passively (opportunistic IMSI paging / IMSI attach only); the reporter's Location Updating Request is actually *uplink* (PD=5 MM) — downlink gives Paging Requests (PD=6 RR) carrying mostly TMSIs, and full TMSI→IMSI mapping needs uplink capture (forcing it is an active IMSI-catcher: illegal, out of scope). BCCH/CCCH are unencrypted so the read is clear-channel; dedicated channels are A5/1 and out of scope. C0 doesn't frequency-hop, so single-ARFCN tuning is enough (2.4 MS/s covers one 200 kHz channel). RTL-SDR (R828D, ~1.7 GHz ceiling) handles GSM850/900 downlink; DCS1800/PCS1900 needs the HackRF. New `gsm-downlink` command, separate from the uplink scanner. Real cost is the dependency — GNU Radio + gr-gsm is heavy and fiddly on a Pi (use a fork tracking GR 3.10); GSMTAP→L3 via pyshark/tshark or a ~150-line hand-rolled GSM 04.08 paging-request parser. Only works where 2G is still live. Requested in #11.
