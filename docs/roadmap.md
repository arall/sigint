...
5. **Server→agent reliable CMD** — mirror the agent's outbox on the server side: seq + ACK + exponential backoff for CMD/CFG. Resolves the "CMD retries" In-flight item and lets operators click Start once.
