...
- **Opportunistic noise-floor anomaly detector** — complement `sdr.py jammer` (dedicated SDR required) with a passive pass over every scanner's `.db` that computes per-band baselines from already-logged `noise_floor_db` values and fires alerts when a band spikes. Zero extra hardware — catches jamming on bands where a scanner is already running.
